...
// Gerar 100 gotas com posições e tempos aleatórios
for (let i = 0; i < 100; i++) {
    ...
    <!-- overlay da chuva (vazio, gerado via JS) -->
        ...
        <nav class="bg-light d-flex gap-2 p-3 align-items-center"
            ...
            <div class="ms-auto d-flex gap-2">
                <Link v-if="$page.props.auth.user" :href="route('dashboard')" class="bg-dark btn btn-outline-light btn-sm">
                Dashboard
                </Link>
                ...
                <template v-else>
                    ...
                    <Link v-if="canRegister" :href="route('register')" class="bg-dark btn btn-outline-light btn-sm" hidden>
                    Register
                    </Link>
                </template>
            </div>
...
html, body {
  ...
  width: 100%;
  height: 100%;
  background: #0a0a0a; /* fundo noturno */
  overflow: hidden;
}
...
.c1 { width: 520px; height:140px; top:8%;  left:-30%; opacity:0.95;
      animation: floatRight 60s linear infinite; }
.c1::before{ left:-10%; width:45%; height:120%; }
.c1::after { left:50%;  width:55%; height:110%; top:-20%; }

.c2 { width: 380px; height:110px; top:28%; left:-40%; opacity:0.7;
      animation: floatRight 90s linear infinite -10s; }
.c2::before{ left:5%;  width:55%; height:130%; }
.c2::after { left:48%; width:48%; height:105%; top:-25%; }

.c3 { width: 680px; height:180px; top:55%; left:-50%; opacity:0.6;
      animation: floatRight 120s linear infinite -30s; }
.c3::before{ left:-8%; width:50%; height:125%; }
.c3::after { left:52%; width:60%; height:115%; top:-18%; }

.c4 { width: 240px; height:80px; top:75%; left:-20%; opacity:0.5;
      animation: floatRight 45s linear infinite -5s; }
.c4::before{ left:8%;  width:48%; height:130%; }
.c4::after { left:48%; width:42%; height:110%; top:-22%; }
